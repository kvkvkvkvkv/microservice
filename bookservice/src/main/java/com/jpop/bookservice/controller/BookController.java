...
public class BookController {
  ...
  @Inject BookServiceSpecification bookServiceSpecification;

  @Inject BookMapper bookMapper;

  @GetMapping("/")
  public ResponseEntity<List<BookDto>> getBook() {
    return ResponseEntity.ok(bookServiceSpecification.getBook()
                                 .stream()
  ...
  public ResponseEntity<BookDto> getBook(@PathVariable("book_id") Long id) {
    return ResponseEntity.ok(
        bookMapper.convertToDto(bookServiceSpecification.getBook(id)));
  }

  @PostMapping("/")
  public ResponseEntity<BookDto> addBook(@RequestBody BookDto book) {
    Book newBook =
        bookServiceSpecification.saveBook(bookMapper.convertToEntity(book));
    URI location = ServletUriComponentsBuilder.fromCurrentContextPath()
                       .path("{book_id}")
                       .buildAndExpand(newBook.getId())
                       .toUri();
    return ResponseEntity.created(location).body(
        bookMapper.convertToDto(newBook));
  }

  @PutMapping("/{book_id}")
  public ResponseEntity<BookDto> updateBook(@RequestBody BookDto book,
                                            @PathVariable("book_id") Long id) {
    Book updateBook = bookServiceSpecification.updateBook(
        bookMapper.convertToEntity(book), id);
    return new ResponseEntity<>(bookMapper.convertToDto(updateBook),
                                HttpStatus.OK);
  }
